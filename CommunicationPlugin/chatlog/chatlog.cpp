    ...
    : QGraphicsView(parent)
{
    ...
    setAcceptDrops(false);   //不允许拖拽
    setAlignment(Qt::AlignTop | Qt::AlignLeft);     //左上角对齐
    setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);   //关闭谁水平滚动条
    setDragMode(QGraphicsView::NoDrag);
    setViewportUpdateMode(MinimalViewportUpdate);
    //setContextMenuPolicy(Qt::CustomContextMenu);    //定制菜单发送
    setBackgroundBrush(QBrush(Qt::white, Qt::SolidPattern));

    // The selection rect for multi-line selection
    selGraphItem = scene->addRect(0,0,0,0,selectionRectColor.darker(120),selectionRectColor);  //选择时矩形的颜色
    ...
    selectionTimer->setInterval(1000/30);     //时间间隔
    selectionTimer->setSingleShot(false);    //不是信号槽定时器
...
void ChatLog::clearSelection()
{
    ...
    selectionMode = None;
    emit selectionChanged();   //设置复制按钮不可用

    updateMultiSelectionRect();
}

QRect ChatLog::getVisibleRect() const   //获取可见窗体矩形的大小
{
...
{
    setSceneRect(calculateSceneRect()); // 设置场景矩形
}
...
void ChatLog::reposition(int start, int end, qreal deltaY)  //垂直方向上移动
{
    if (lines.isEmpty())
        return;
...
void ChatLog::removeFirst()
{
    lines.first()->removeFromScene();
    lines.removeFirst();
    checkVisibility();
}
...
QString ChatLog::getSelectedText() const
{
    ...
    else if (selectionMode == Multi)
    {
        ...
        for (int i=selFirstRow; i<=selLastRow; ++i)
        {
            QString timestamp = lines[i]->content[2]->getText();
            QString author = lines[i]->content[0]->getText();
            QString msg = lines[i]->content[1]->getText();

            out += QString(out.isEmpty() ? "%1 %2 %3" : "\n%1 %2 %3").arg(author, msg,timestamp);
        }
...
void ChatLog::copySelectedText(bool toSelectionBuffer)  //待解决
{

    mimeData=new QMimeData;
    cliptext=getSelectedText();
    qDebug()<<cliptext;
    /*if(cliptext.contains("temp_picture/mypic", Qt::CaseSensitive))
    {
        //Image.startsWith()
        qDebug()<<"temp_picture is true";
        QClipboard* clipboard = QApplication::clipboard();
        clipboard->setImage(QImage(cliptext));
    ...
    }*///else
    {
        //text.startsWith()
        QClipboard* clipboard = QApplication::clipboard();
        if (clipboard && !cliptext.isNull())
        {

            mimeData->clear();
            mimeData->setHtml(cliptext);
...
/*void ChatLog::wheelEvent(QWheelEvent *event)
{
    ...
    Core::getInstance()->updateFiletransfer(); //更新文件传输窗体
}*/
