...
public:
    ...
    // rawText: does not contain html code 返回的文本

    Text(const QString& txt = "", QFont font = QFont(), bool enableElide = false, const QString& rawText = QString(), const QColor c = Qt::black);
    virtual ~Text();

    void setText(const QString& txt);             //设置文本

    virtual void setWidth(qreal width) final;      //设置宽度

    virtual void selectionMouseMove(QPointF scenePos) final;     //选择鼠标移动
...
protected:
    ...
    QSizeF idealSize();
    int cursorFromPos(QPointF scenePos, bool fuzzy = true) const;     //光标转换
