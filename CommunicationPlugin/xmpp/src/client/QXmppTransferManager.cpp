...
QString QXmppTransferFileInfo::range() const
{
    return d->range;
}

void QXmppTransferFileInfo::setRange(const QString &range)
{
    d->range = range;
}

qint64 QXmppTransferFileInfo::offset() const
{
    return d->offset;
}
void QXmppTransferFileInfo::setOffset(const qint64 &offset)
{
    d->offset = offset;
}

qint64 QXmppTransferFileInfo::length() const
{
    return d->length;
}
void QXmppTransferFileInfo::setLength(const qint64 &length)
{
    d->length = length;
}
...
void QXmppTransferFileInfo::parse(const QDomElement &element)
{
   ...
   /******************************add******************************************/
    ...
    d->offset = rangeElement.attribute("offset").toInt();
    d->length = rangeElement.attribute("length").toInt();
   /***************************************************************************/
}
...
void QXmppTransferFileInfo::toXml(QXmlStreamWriter *writer) const
{
    ...
    if (!d->range.isEmpty())
    {
        ...
        //if(d->length >= 0)
            writer->writeAttribute("length", QString::number(d->length));
...
void QXmppTransferJob::accept(QIODevice *iodevice)
{
    ...
    {
        d->iodevice = iodevice;
        qDebug()<<"file name is:"<<d->fileInfo.name();
        setState(QXmppTransferJob::StartState);
    }
}
...
/**********************************add**************************************/
void QXmppTransferJob::setBreakpoint(const QString &bp)
{
    d->desc = bp;
}

QString QXmppTransferJob::Breakpoint() const
{
    return d->desc;
}

void QXmppTransferJob::setStoryPath(const QString &path)
{
    d->storyPath = path;
}

QString QXmppTransferJob::storyPath() const
{
    return d->storyPath;
}

void QXmppTransferJob::setReturnFileSize(const qint64 &size)
{
    d->returnsize = size;
}

qint64 QXmppTransferJob::returnFileSize() const
{
    return d->returnsize;
}
/**********************************************************************************/
...
/***************************************add********************************************/
int QXmppTransferJob::breakPoint() const
{
    return d->desc.toInt();
}

qint64 QXmppTransferJob::fileOffset() const
{
    return d->fileInfo.offset();
}

// edit
void QXmppTransferJob::transferCancle() const
{
    d->socksSocket->flush();
    d->socksSocket->close();
    d->iodevice->close();
...
//    this->terminate(QXmppTransferJob::AbortError);
}

void QXmppTransferJob::transferPause() const
{
...
void QXmppTransferJob::transferContinue() const
{
    d->iodevice->open(QIODevice::ReadWrite);
}

void QXmppTransferJob::transferSetStoryPath(const QString &path)
{
    d->storyPath = path;
    testStoryPath = path;
}
//发送端获取返回文件保存路径
QString QXmppTransferJob::transferGetStoryPath() const
{
    return d->storyPath;
}
//接收端读取文件保存路径
QString QXmppTransferJob::readStoryPath() const
{
    return d->fileInfo.description();
}
/*************************************************************************************/
/// \endcond
...
/***************************************************add*****************************************/
    //本地获取文件大小
//qDebug()<<"----->"<<d->fileInfo.name()<<"----->"<<d->fileInfo.offset()<<"----->"<<d->storyPath<<"---->"<<testStoryPath;
    QFile sf;
    sf.setFileName(d->storyPath);
    if(d->fileInfo.offset() > 0)
    {
        if(sf.size() == 0)
        {
            d->done = 0;
            qDebug()<<"sf.size()---->"<<sf.size();
        }
        else
        {
            d->iodevice->seek(d->fileInfo.offset());
            d->done = d->fileInfo.offset();
        }
    }
    else
    {
...
void QXmppTransferOutgoingJob::startSending()
{
    ...
    if(d->desc.toInt() > d->fileInfo.size())
    {
        qDebug()<<"the filally file is exist or damaged!";
        //return;
    }
    else
    {
/**************************************************add*************************************************/
        qDebug()<<"BreakPoint data is:"<<d->desc<<"offset is :"<<d->fileInfo.offset();
        //move ops
        /***************************************************************************************************************************/
        //当发送端没有检测到此次你发送文件为断点续传文件时，判断接受端返回的与本次传输相匹配的文件大小，
        //如果大于零，说明发送端异常结束没有及时保存文件断点,将断点值设置为返回数据
        //如果等于零，说明此次传输的文件确实是第一次传输
        /***************************************************************************************************************************/
        if(d->fileInfo.offset() == 0)
        {
            if(d->desc.toInt() > 0)
            {
                d->iodevice->seek(d->desc.toInt());
                d->done = d->desc.toInt();
            }
        }
        else
...
void QXmppTransferOutgoingJob:: _q_sendData()
{
    if (d->state != QXmppTransferJob::TransferState)
    {
        qDebug()<<"this is send trst!!!:state change!";
        return;
    }
    ...
    {
        qDebug()<<"this is send trst!!!:no socket!";
        return;
    }
    ...
    if (d->fileInfo.size() && d->done >= d->fileInfo.size())
    {
        ...
        {
            qDebug()<<"this is send trst!!!:bytesToWrite error!";
            terminate(QXmppTransferJob::NoError);
        }
        return;
    }
...
/******************************add************************/
    job->setBreakpoint(iq.desc());//liu 20161107 对端返回的断点值
/*********************************************************/
...
bool QXmppTransferManager::handleStanza(const QDomElement &element)
{
    ...
    /******************************add*************************************/
        byteStreamIq.setDesc(QString::number(breakpoint));
    /**********************************************************************/
    ...
    {
    /***********************************add****************************************/
        QString ds = element.attribute("desc");
        breakpoint = ds.toInt();
    /*******************************************************************************/
...
void QXmppTransferManager::_q_jobStateChanged(QXmppTransferJob::State state)
{
    ...
    //response.setDesc(QString::number(breakpoint));
    /************************************add*****************************/
    response.setDesc(QString::number(job->returnFileSize()));
	/*********************************************************************/
    client()->sendPacket(response);
    // notify user
    emit jobStarted(job);
...
QXmppTransferJob *QXmppTransferManager::sendFile(const QString &jid, const QString &fn, const QString &description)
{
    ...
    QStringList str = fn.split("#");
    QString filePath = str.at(0);
    const qint64 a = str.at(1).toLongLong();
    const qint64 b = 0 ;
	...
	/***************************************add*************************************/
    fileInfo.setRange("range");
    fileInfo.setOffset(a);
    fileInfo.setLength(b);
	/*******************************************************************************/
...
{
    QXmppTransferJob *job = d->getOutgoingJobByRequestId(iq.from(), iq.id());
    if (!job || job->state() != QXmppTransferJob::OfferState)
        return;
