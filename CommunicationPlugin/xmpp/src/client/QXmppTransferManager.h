...
class QXMPP_EXPORT QXmppTransferJob : public QXmppLoggable
{
    ...
    Q_PROPERTY(QString storyPath READ storyPath CONSTANT)
...
public:
    ...
    enum State
    {
        ...
        //edit 
        CancelState = 4    ///< the transfer is canceled
    };
    ...
    void setBreakpoint(const QString &);

    qint64 fileOffset() const;

    QString storyPath() const;
    void setStoryPath(const QString &);

    void setReturnFileSize(const qint64 &);
    qint64 returnFileSize() const;
...
public:
    ...
    void transferCancle() const;
    void transferPause() const;         //test 勿使用
    void transferContinue() const;      //test 勿使用

    void transferSetStoryPath(const QString &);
    QString transferGetStoryPath() const;
    QString readStoryPath() const;

	QString testStoryPath;
...
/// using either XEP-0065: SOCK S5 Bytestreams or XEP-0047: In-Band Bytestreams.
///
...
public slots:
    QXmppTransferJob *sendFile(const QString &jid, const QString &fn, const QString &description = QString());
